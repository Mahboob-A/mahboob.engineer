...
| 30  — 6th line on How the form gets used | 3313–3374 | "Nothing here trains an AI model." |
| 31  — OG / favicon ChatGPT prompts + integration plan | 3375–3500 | 5 ChatGPT image-gen prompts + 7-commit plan docs |
| 31.2 — OG + favicon asset integration | 3501–end | Static PNGs in /public, @vercel/og removed |
...
**Phase:** 31.2 — replace @vercel/og with real designed
assets
...
**Goal:** Drop the @vercel/og dynamic generators
(`app/icon.tsx`, `app/opengraph-image.tsx`) and replace
them with the ChatGPT-generated + RealFaviconGenerator
assets that ship as static PNGs under `/public/`.

**Project OG cards clarification** — earlier (Phase 31
plan) I described `og-project.png` as a single template.
The user pushed back: they already generated 3 per-project
OG cards (algocode, movio, drishti-ai) from the project
prompt. So the real plan is **one PNG per project**. As
the user generates more cards, the slug joins a lookup
set in `lib/og-helpers.ts`; anything missing falls back
to `/og-default.png` so incomplete coverage doesn't
404 share previews.

---

## T31.2.1 — Asset layout (favicons + manifest)
...
10 PNG / ICO / webmanifest assets moved from
`/public/favicon_io/` (a staging folder) up to `/public/`
root, so they sit at the canonical URLs the browser +
next.js metadata-file convention expect:

- `/public/favicon-16x16.png` (browser tab, small)
- `/public/favicon-32x32.png` (browser tab, default)
- `/public/favicon.ico` (legacy / address-bar)
- `/public/apple-touch-icon.png` (iOS home screen)
- `/public/android-chrome-192x192.png` (Android home)
- `/public/android-chrome-512x512.png` (Android splash)
- `/public/manifest.webmanifest` (PWA manifest)
- `/public/og-default.png` (default OG card)
- `/public/og-writing.png` (writing surface OG)
- `/public/og-projects/{algocode,movio,drishti-ai}.png`
  (first 3 of 12 per-project OGs — generated by the
  user from Phase 31's Prompt 4)

The `/public/favicon_io/` staging folder is removed.

---

## T31.2.2 — Wire assets, drop generators, remove dep
...
- **`app/layout.tsx`** — `icons` object updated:
  - `icons.icon`: array of 2 PNG entries (32×32 + 16×16)
    instead of the dynamic `/icon` route.
  - `icons.shortcut`: `/favicon.ico` (real file now,
    previously 404).
  - `icons.apple`: `/apple-touch-icon.png` (real file).
  - `manifest: "/manifest.webmanifest"` (PWA).
- **`lib/og-helpers.ts`** — `ogUrlFor(args)` rewritten:
  - Takes `{ title, description, surface, slug }`.
  - `surface: "default"` → `/og-default.png`.
  - `surface: "writing"` → `/og-writing.png`.
  - `surface: "project"` with a `slug` →
    `/og-projects/{slug}.png` if generated, fallback
    to `/og-default.png` otherwise.
  - New `GENERATED_PROJECT_OG_SLUGS` Set =
    `{ algocode, movio, drishti-ai }` — the lookup table
    for which per-project cards exist.
- **`lib/metadata.ts`** — `ogImage()` helper now takes
  `{ title, description, surface, slug }`. Each
  `pageMetadata` / `projectMetadata` / `blogMetadata`
  helper routes to the right surface:
  - `pageMetadata(title, description, surface = "default")`.
  - `projectMetadata(...)` passes `surface: "project", slug`.
  - `blogMetadata(...)` passes `surface: "writing"`.
- **`app/writing/page.tsx`** — calls
  `pageMetadata("Writing", desc, "writing")` so the
  page-level OG card is `/og-writing.png` (not the
  default).
- **`app/icon.tsx` deleted** — Next.js metadata-file
  convention now resolves to `/public/favicon-32x32.png`
  via the `icons.icon` array.
- **`app/opengraph-image.tsx` deleted** — `/opengraph-image`
  no longer in build output.
- **`package.json` + `pnpm-lock.yaml`** —
  `@vercel/og` removed (-23 packages including
  `@vercel/satori`, `@resvg/resvg-wasm`,
  `@shuding/opentype.js` and friends).

### Decisions

- **Per-project OG lookup, not template** — the
  Phase 31 integration doc described `og-project.png`
  as a single template; the user pushed back, correctly
  noting that real per-project cards are better. The
  system now looks up the project PNG by slug and falls
  back to the default OG. As more per-project PNGs are
  generated, just add the slug to
  `GENERATED_PROJECT_OG_SLUGS`.
- **Favicon_root /icons in `app/layout.tsx`** uses 2
  entries (16 + 32), not a single string. Modern
  browsers pick the best match; legacy crawlers fall
  back to `/favicon.ico`.
- **No `apple-touch-icon-192x192.png` or
  `apple-touch-icon-512x512.png`** — iOS 14+ reads only
  the 180×180 from `<link rel="apple-touch-icon">`,
  Android Chrome launcher uses its own
  `android-chrome-*` PNGs. One apple-touch icon ships,
  and the doc explicitly notes the 512/192 are
  deliberately skipped.
- **`@vercel/og` removed in the same commit as the
  generator deletion** — no orphan-dep window. ~600 KB
  server bundle savings.
- **9 remaining per-project OG cards** (datalineage-doctor,
  cutetube, airpass, pulumi-infra, imgtwist,
  load-balancer, prostream, drishti-ai, unthink —
  wait, drishti-ai is already done) ship incrementally as
  the user generates them. No new commits needed beyond
  the asset + one slug-add to `GENERATED_PROJECT_OG_SLUGS`.

### Verified

- `pnpm typecheck` → clean.
- `pnpm build` → 18 routes + middleware, 0 warnings.
  `/icon` and `/opengraph-image` no longer in the build
  output.
- `pnpm install` removed 23 packages; final `package.json`
  has no @vercel/og entry.
- Live SSR smoke (HTML inspection once user runs
  `pnpm dev`):
  - `/` → `<meta property="og:image" content="https://mahboob.engineer/og-default.png">`.
  - `/writing` → `...og-writing.png`.
  - `/work/algocode` → `...og-projects/algocode.png`.
  - `/work/taply` (no per-project PNG yet) →
    `...og-default.png` (fallback works).
...
- **9 of 12 per-project OG cards still missing** —
  ...
  The user runs Prompt 4 (Phase 31 doc) per project,
  drops the PNG into `/public/og-projects/{slug}.png`,
  and adds the slug to
  `GENERATED_PROJECT_OG_SLUGS`. Each is one slug-add
  + one commit. No urgency.
- **`/public/apple-icon.png` was a stale PNG
  already shipped** by Phase 6 (T6.5). It's removed in
  the same commit; the metadata-file convention that
  generated it is gone. Already-cached OG previews
  from crawlers may briefly show the old apple-icon
  but new requests hit the new apple-touch-icon.png.
- **No PWA-ready `start_url` etc.** in the manifest —
  the site isn't fully PWA yet (offline support,
  install prompts). The manifest is wired so the
  browser stops 404'ing it; full PWA work is a future
  phase.
- **No `apple-touch-icon-precomposed.png`** — iOS
  dropped support for this in iOS 7. The new
  single apple-touch-icon.png is what every iOS
  version since has expected.

---

Phase 31.2 status: **done**. 9 per-project OG cards remain
as an incremental follow-up; no more code changes needed
unless the design system changes.
